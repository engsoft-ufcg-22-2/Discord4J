...
class ReconnectFailEventTest {
    /**
     * Methods under test:
     *
     * <ul>
     *   <li>{@link ReconnectFailEvent#ReconnectFailEvent(GatewayDiscordClient, ShardInfo, long)}
     *   <li>{@link ReconnectFailEvent#toString()}
     *   <li>{@link ReconnectFailEvent#getCurrentAttempt()}
     * </ul>
     */
    @Test
    void testConstructor() {
        ...
        ReconnectFailEvent actualReconnectFailEvent = new ReconnectFailEvent(new GatewayDiscordClient(null,
                gatewayResources, null, null, voiceConnectionFactory, entityRetrievalStrategy, new HashSet<>()),
                mock(ShardInfo.class), 1L);
        String actualToStringResult = actualReconnectFailEvent.toString();
        assertEquals(1L, actualReconnectFailEvent.getCurrentAttempt());
        assertEquals("Gateway reconnect attempt failed", actualToStringResult);
    }
}
