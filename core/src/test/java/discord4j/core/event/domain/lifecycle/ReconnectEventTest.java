...
class ReconnectEventTest {
    /**
     * Methods under test:
     *
     * <ul>
     *   <li>{@link ReconnectEvent#ReconnectEvent(GatewayDiscordClient, ShardInfo, long)}
     *   <li>{@link ReconnectEvent#toString()}
     *   <li>{@link ReconnectEvent#getCurrentAttempt()}
     * </ul>
     */
    @Test
    void testConstructor() {
        ...
        ReconnectEvent actualReconnectEvent = new ReconnectEvent(new GatewayDiscordClient(null, gatewayResources, null,
                null, voiceConnectionFactory, entityRetrievalStrategy, new HashSet<>()), mock(ShardInfo.class), 1L);
        String actualToStringResult = actualReconnectEvent.toString();
        assertEquals(1L, actualReconnectEvent.getCurrentAttempt());
        assertEquals("Gateway successfully reconnected", actualToStringResult);
    }
}
